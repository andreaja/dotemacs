...
(defun midje-align-tabular-facts ()
  (interactive)
  (search-forward-regexp "tabular[\\s-]*\\(qwe\\)")
  ;; (search-forward-regexp "(tabular[.\n]+\\(\\?.*\\)")
  (let ((start (match-beginning 1))
        (end (match-end 1)))
    (align-regexp start end "\\(\\s-*\\)" 1 1 t)))

(defun midje-align-arrows ()
  "Aligns => and =not=> in the current fact."
  (interactive)
  (when (midje-on-fact?)
    (save-excursion
      (mark-defun)
